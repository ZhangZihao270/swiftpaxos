...
// TestExp31ConfigFile tests that configs/exp3.1-base.conf parses correctly
func TestExp31ConfigFile(t *testing.T) {
	c, err := Read("../configs/exp3.1-base.conf", "test")
	if err != nil {
		t.Fatalf("Failed to parse exp3.1-base.conf: %v", err)
	}
...
// TestExp32ConfigFile tests that configs/exp3.2-base.conf parses correctly
func TestExp32ConfigFile(t *testing.T) {
	c, err := Read("../configs/exp3.2-base.conf", "test")
	if err != nil {
		t.Fatalf("Failed to parse exp3.2-base.conf: %v", err)
	}
	if c.Protocol != "curpht" {
		t.Errorf("Protocol = %s, want curpht", c.Protocol)
	}
