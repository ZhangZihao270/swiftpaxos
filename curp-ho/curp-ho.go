...
// When markWeakExecuted advances the seqnum for a client, it closes the broadcast
// channel, waking all waiters. Each waiter re-checks its condition.
...
// If no channel exists, creates one.
...
// Closing the channel wakes all goroutines waiting on it.
