...
// --- Phase 43.2b: Priority fast-path tests ---

// TestPriorityFastPathPattern verifies that the non-blocking select pattern
// used in the run loop correctly prioritizes one channel over others.
func TestPriorityFastPathPattern(t *testing.T) {
	priorityCh := make(chan int, 10)
	normalCh := make(chan int, 10)
	...
	// Drain using the same pattern as the run loop:
	// non-blocking priority check, then blocking select on both
	for i := 0; i < 10; i++ {
		select {
		case <-priorityCh:
			priorityCount++
	...
	if priorityCount != 5 {
		t.Errorf("priorityCount = %d, want 5", priorityCount)
	}
	if normalCount != 5 {
		t.Errorf("normalCount = %d, want 5", normalCount)
	}
}

// TestPriorityFastPathDrainsBeforeNormal verifies that when priority channel
// has messages, they are all consumed before any normal messages.
func TestPriorityFastPathDrainsBeforeNormal(t *testing.T) {
	priorityCh := make(chan int, 10)
	normalCh := make(chan int, 10)

	// Fill priority with 3 items, normal with 3 items
	for i := 0; i < 3; i++ {
		priorityCh <- i
		normalCh <- i + 100
	}
	...
	// First 3 should be priority (fast-path drains them first)
	for i := 0; i < 3; i++ {
		if order[i] != "priority" {
			t.Errorf("order[%d] = %s, want priority", i, order[i])
		}
	}
	// Remaining 3 should be normal
	for i := 3; i < 6; i++ {
		if order[i] != "normal" {
			t.Errorf("order[%d] = %s, want normal", i, order[i])
		}
	}
}

// TestPriorityFastPathEmptyChannelFallsThrough verifies that when the priority
// channel is empty, the pattern falls through to the normal select.
func TestPriorityFastPathEmptyChannelFallsThrough(t *testing.T) {
	priorityCh := make(chan int, 10)
	normalCh := make(chan int, 10)
	...
	select {
	case <-priorityCh:
		t.Fatal("Should not receive from empty priority channel")
	default:
	}

	select {
	case <-priorityCh:
		t.Fatal("Should not receive from empty priority channel in main select")
	case v := <-normalCh:
		result = v
	}
...
// TestCausalProposeChanIsBuffered verifies the channel backing the fast-path
// is buffered (required for non-blocking receives to work under load).
func TestCausalProposeChanIsBuffered(t *testing.T) {
	r := newTestReplicaForDesc(true)
