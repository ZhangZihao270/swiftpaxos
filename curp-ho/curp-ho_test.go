...
// TestCausalProposeChanIsBuffered verifies the causalProposeChan channel
// is buffered (required for high-throughput message processing in the run loop).
...
// TestMarkAllSentNoOp verifies that MarkAllSent is a no-op and doesn't panic.
func TestMarkAllSentNoOp(t *testing.T) {
	c := newTestClient(0)
