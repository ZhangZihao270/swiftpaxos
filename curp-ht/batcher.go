...
//   - 0μs:    Lowest latency, smaller batches, more syscalls (default)
//   - 50μs:   Balanced, 2-3x larger batches, moderate latency increase
//   - 150μs:  Highest throughput, largest batches, higher latency (Phase 32 optimal)
//
// See docs/phase-18.5-batcher-analysis.md and docs/phase-32.3-network-batching.md
type Batcher struct {
	...
	// 0 = immediate send (zero-delay batching, backward compatible)
	// 50-150μs = wait for more messages (adaptive batching, Phase 32)
	batchDelayNs int64
...
func NewBatcher(r *Replica, size int) *Batcher {
	b := &Batcher{
		...
		batchDelayNs: 0, // Default: zero-delay batching (backward compatible)
	...
	go func() {
		for !r.Shutdown {
			select {
			case op := <-b.acks:
				// Apply batching delay if configured (Phase 32)
				if b.batchDelayNs > 0 {
...
// SetBatchDelay sets the batching delay in nanoseconds (Phase 32)
// 0 = immediate send, 50000 = 50μs delay, 150000 = 150μs delay
...
func (b *Batcher) GetAvgBatchSize() float64 {
	...
	totalMsgs := atomic.LoadUint64(&b.totalAcks) + atomic.LoadUint64(&b.totalAccepts)
	return float64(totalMsgs) / float64(batches)
}
