...
# Phase 100: Exp 1.1 — Raft vs Raft-HT with All Optimizations (5r/5m/3c)
#
# Fair comparison of Raft (strong-only) vs Raft-HT (hybrid, weakRatio=50).
# Both protocols have the same optimizations applied:
# - broadcastAppendEntries optimization (single logMu + entries sharing)
# - handleAppendEntriesReplyBatch (reply batching)
# - ReplyProposeTSDelayed (symmetric WAN delay for Raft)
# - Raft-HT: weak propose batch control
#
# Two write ratios: writes=5 (5%) and writes=50 (50%).
# 5 replicas on 5 machines, 3 clients on .101/.103/.104
# reqs=3000, networkDelay=25, commandSize=100, batchDelayUs=150
#
# Usage: bash scripts/eval-phase100.sh [output-dir] [write-filter] [protocol-filter]
#   write-filter: all (default), w5, w50
#   protocol-filter: all (default), raft, raftht

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase100-$DATE}"
WRITE_FILTER="${2:-all}"
PROTO_FILTER="${3:-all}"
...
if [[ ${#PROTOCOLS[@]} -eq 0 ]]; then
    echo "ERROR: No protocols matched filter '$PROTO_FILTER' (valid: all, raft, raftht)"
    exit 1
fi
...
run_benchmark() {
    ...
    mkdir -p "$out_dir"
    timeout 300 scripts/run-multi-client.sh -d -c "$config" -t "$threads" --startup-delay 25 -o "$out_dir" \
...
log "Phase 100: Exp 1.1 — Raft vs Raft-HT (All Optimizations) (5r/5m/3c)"
...
for wc_spec in "${WRITE_CONFIGS[@]}"; do
    ...
    for proto_spec in "${PROTOCOLS[@]}"; do
        ...
        proto_config="/tmp/eval-phase100-${name}-${wc_label}-$$.conf"
...
log ""
log "Phase 100 complete! Results in $BASE_DIR/"
