...
#
# Sweeps thread count for EPaxos with two write ratios (5%, 50%).
# 5 replicas on 5 machines, 3 clients on .101/.103/.104
# reqs=3000, networkDelay=25, commandSize=100
#
# Usage: bash scripts/eval-phase104-epaxos.sh [output-dir] [write-filter]
...
DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase104-$DATE}"
WRITE_FILTER="${2:-all}"
...
# EPaxos is all-strong (weakRatio=0, weakWrites=0), so only writes matters
...
log "Phase 104: EPaxos Throughput-Latency Benchmark (5r/5m/3c)"
...
for wc_spec in "${WRITE_CONFIGS[@]}"; do
    ...
    proto_config="/tmp/eval-phase104-epaxos-${wc_label}-$$.conf"
    make_config "$proto_config" "$writes"

    log "=== Protocol: epaxos (weakRatio=0, writes=$writes) ==="
...
log ""
log "Phase 104 complete! Results in $BASE_DIR/"
