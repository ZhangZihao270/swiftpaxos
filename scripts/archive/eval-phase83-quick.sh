...
# Phase 83.4: Quick Verification — CURP correctness fix impact
#
# Runs t=32 and t=64 only for all 3 CURP protocols to verify
# throughput drops to Phase 76 levels after MSync recovery bypass removal.
#
# Expected Phase 76 results (pre-bug):
#   baseline: ~25K (t=32), ~27K (t=64)
#   curpht:   ~43K (t=32), ~44K (t=64)
#   curpho:   ~80K (t=32), ~97K (t=64)
#
# Usage: bash scripts/eval-phase83-quick.sh [output-dir]

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r-phase83-quick-$DATE}"
EXP_DIR="$BASE_DIR/exp3.1"
THREAD_COUNTS=(32 64)
MAX_RETRIES=2

# Use the 5-replica config
CONFIG="/tmp/eval-phase83-quick-$$.conf"
...
log "Phase 83.4: Quick Verification (t=32, t=64)"
...
log ""
log "Phase 83.4 quick verification complete! Results: $BASE_DIR/summary-exp3.1.csv"
