...
# Phase 91: Force-deliver removed — re-validate all protocols
#
# 5 replicas on 5 machines, 3 clients on .101/.103/.104, reqs=3000.
# Protocols: curpht first (91c), then curp-baseline and curpho (91e).
#
# Usage: bash scripts/eval-phase91.sh [output-dir] [protocols]
# protocols: "curpht" (default) or "all" (curp-baseline + curpht + curpho)

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase91-$DATE}"
PROTO_MODE="${2:-curpht}"
EXP_DIR="$BASE_DIR/exp3.1"
THREAD_COUNTS=(4 16 32 64 96)
MAX_RETRIES=2

# Use the 5-replica 3-client config with reqs=3000
CONFIG="/tmp/eval-phase91-$$.conf"
cp benchmark-5r-5m-3c.conf "$CONFIG"
sed -i -E "s/^reqs:.*$/reqs:        3000/" "$CONFIG"
trap 'rm -f "$CONFIG"' EXIT

if [[ "$PROTO_MODE" == "all" ]]; then
    declare -a PROTOCOLS=(
...
log "Phase 91: Force-deliver removed — re-validate ($PROTO_MODE)"
...
log ""
log "Phase 91 complete! Results: $BASE_DIR/summary-exp3.1.csv"
