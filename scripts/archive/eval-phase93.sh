...
# Phase 93: Exp 3.1 — CURP throughput vs latency on 5r/5m/3c cluster
#
...
#
# Usage: bash scripts/eval-phase93.sh [output-dir] [protocol-filter]
#   protocol-filter: "all" (default), "curp", "curpht", "curpho"

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase93-$DATE}"
PROTO_FILTER="${2:-all}"
...
# Create per-protocol temp configs from the base template
...
log "Phase 93: Exp 3.1 — CURP throughput vs latency (5r/5m/3c)"
...
for proto_spec in "${PROTOCOLS[@]}"; do
    ...
    proto_config="/tmp/eval-phase93-${name}-$$.conf"
...
log ""
log "Phase 93 complete! Results: $BASE_DIR/summary-exp3.1.csv"
