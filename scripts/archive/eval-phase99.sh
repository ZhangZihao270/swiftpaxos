...
# Phase 99: Exp 3.1 — EPaxos-HO Throughput vs Latency (5r/5m/3c)
#
# EPaxos-HO: hybrid consistency on leaderless EPaxos.
# Strong ops: 2-RTT EPaxos consensus. Weak ops: 1-RTT causal commit.
# Also runs EPaxos baseline (strong-only) for comparison.
#
...
#
# Usage: bash scripts/eval-phase99.sh [output-dir] [write-filter] [protocol-filter]
#   write-filter: all (default), w5, w50
#   protocol-filter: all (default), epaxosho, epaxos

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase99-$DATE}"
WRITE_FILTER="${2:-all}"
PROTO_FILTER="${3:-all}"
...
# EPaxos-HO: hybrid (weakRatio=50), leaderless
# EPaxos: strong-only baseline (weakRatio=0), leaderless
...
if [[ ${#PROTOCOLS[@]} -eq 0 ]]; then
    echo "ERROR: No protocols matched filter '$PROTO_FILTER' (valid: all, epaxosho, epaxos)"
    exit 1
fi
...
make_config() {
    ...
    # EPaxos variants are leaderless, not fast-path
    sed -i -E "s/^fast:.*$/fast:       false/" "$out"
}
...
log "Phase 99: Exp 3.1 — EPaxos-HO Throughput vs Latency (5r/5m/3c)"
...
for wc_spec in "${WRITE_CONFIGS[@]}"; do
    ...
    EXP_DIR="$BASE_DIR/exp3.1-${wc_label}"
    mkdir -p "$EXP_DIR"
    ...
    for proto_spec in "${PROTOCOLS[@]}"; do
        ...
        proto_config="/tmp/eval-phase99-${name}-${wc_label}-$$.conf"
    ...
    bash scripts/collect-results.sh throughput "$EXP_DIR" "$BASE_DIR/summary-exp3.1-${wc_label}.csv"
done

log ""
log "Phase 99 complete! Results in $BASE_DIR/"
