#!/bin/bash
#
# Phase 47 Validation Benchmark Sweep
#
# Validates the restored CURP-HO fast path (1-RTT strong commands)
# with per-replica mutex protection (sendProposeSafe):
# 1. S-Med ≈ 51ms at all thread counts (1-RTT fast path restored)
# 2. Throughput ≥ Phase 42 reference (3.5K at 2t, 71K at 96t)
...
#
# Usage: bash scripts/run-phase47-validation.sh
...
SWEEP_DIR="results/phase47-sweep-$(date +%Y%m%d-%H%M%S)"
EVAL_FILE="evaluation/phase47-results.md"
...
log "Phase 47 Validation Benchmark Sweep"
...
|--------:|--------------------:|--------------------:|--------------------:|---------------:|---------------:|
| 2       | 3,551               | 1,336               |                     | 100.19ms       |                |
| 4       | 4,109               | 2,663               |                     | 100.17ms       |                |
| 8       | 14,050              | 5,292               |                     | 100.15ms       |                |
| 16      | 8,771               | 10,546              |                     | 100.09ms       |                |
| 32      | 30,339              | 21,050              |                     | 100.00ms       |                |
| 64      | 34,797              | 42,114              |                     | 99.86ms        |                |
| 96      | 71,595              | 61,494              |                     | 99.81ms        |                |

(Fill in Phase 47 values from the results above)

## Validation Criteria

1. **S-Med ≈ 51ms** at all thread counts — confirms 1-RTT fast path restored (was ~100ms with Fast=false in Phase 46)
2. **Throughput ≥ Phase 42 reference** — confirms fast path improves throughput over Phase 46 leader-only mode
3. **W-P99 < 2ms at 4-16 threads** — confirms Phase 46 async queue optimization preserved
4. **Zero "unknown client message" errors** — confirms per-replica mutex protection prevents writer race
EOF

log ""
log "Phase 47 validation sweep complete!"
