#!/bin/bash
#
# Phase 50 Vanilla Raft Baseline Benchmark
#
# Runs vanilla Raft (all-strong, weakRatio=0) as baseline for Phase 50 comparison.
# Used to validate that Raft-HT throughput >= Raft at all thread counts.
#
# Usage: nohup bash scripts/run-phase50-raft-baseline.sh &
# Output: results/phase50-raft-baseline-*/ + evaluation/phase50-raft-baseline.md
...
SWEEP_DIR="results/phase50-raft-baseline-$(date +%Y%m%d-%H%M%S)"
EVAL_FILE="evaluation/phase50-raft-baseline.md"
...
log "Phase 50 Vanilla Raft Baseline Benchmark"
...
log ""
log "Phase 50 Raft baseline sweep complete!"
