#!/bin/bash
#
# Phase 51 Vanilla Raft Baseline Benchmark (Post Batch-Cap Fix)
#
# Re-runs vanilla Raft (all-strong, weakRatio=0) with the Phase 51 batch size cap fix
# (maxBatchSize=256 in handlePropose). Collects the missing 64-thread and 96-thread data
# that previously failed due to election storms from event loop starvation.
#
# Usage: nohup bash scripts/run-phase51-raft-baseline.sh &
# Output: results/phase51-raft-baseline-*/ + evaluation/phase51-raft-baseline.md
...
SWEEP_DIR="results/phase51-raft-baseline-$(date +%Y%m%d-%H%M%S)"
EVAL_FILE="evaluation/phase51-raft-baseline.md"
...
log "Phase 51 Vanilla Raft Baseline Benchmark (Post Batch-Cap Fix)"
log "Fix: maxBatchSize=256 in handlePropose — prevents election storms at high concurrency"
...
log ""
log "Phase 51 Raft baseline sweep complete!"
