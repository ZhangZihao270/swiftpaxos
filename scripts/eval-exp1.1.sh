...
# Exp 1.1: Raft-HT vs baselines — Throughput vs Latency
#
...
#   pileus         — protocol: pileus, weakRatio: 50
#   pileusht       — protocol: pileusht, weakRatio: 50
#
...
DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-exp1.1-$DATE}"
...
cp "$SUMMARY_CSV" "$WORK_DIR/results/latest/exp1.1.csv" 2>/dev/null || true
log "Updated results/latest/exp1.1.csv"
log "Exp 1.1: Raft, Raft-HT, MongoDB, Pileus, Pileus-HT"
...
cp "$SUMMARY_CSV" "$WORK_DIR/results/latest/exp1.1.csv" 2>/dev/null || true
log "Updated results/latest/exp1.1.csv"
log "Exp 1.1 complete! Results: $SUMMARY_CSV"
