...
#
# Benchmarks the newly ported Orca EPaxos (`epaxos/`) package.
# Same setup as Phase 104 (epaxos-swift) for direct comparison.
# 5 replicas on 5 machines, 3 clients on .101/.103/.104
# reqs=3000, networkDelay=25, commandSize=100
#
# Usage: bash scripts/eval-phase107-epaxos-orca.sh [output-dir] [write-filter]
...
DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase107-$DATE}"
WRITE_FILTER="${2:-all}"
...
# EPaxos (Orca) is all-strong (weakRatio=0, weakWrites=0), so only writes matters
...
make_config() {
    ...
    sed -i -E "s/^protocol:.*$/protocol: epaxos/" "$out"
    sed -i -E "s/^reqs:.*$/reqs:        3000/" "$out"
    sed -i -E "s/^weakRatio:.*$/weakRatio:   0/" "$out"
    sed -i -E "s/^writes:.*$/writes:      $writes/" "$out"
    sed -i -E "s/^weakWrites:.*$/weakWrites:  0/" "$out"
    # EPaxos is leaderless, uses fast path
...
log "Phase 107: EPaxos (Orca port) Throughput-Latency Benchmark (5r/5m/3c)"
...
for wc_spec in "${WRITE_CONFIGS[@]}"; do
    ...
    proto_config="/tmp/eval-phase107-epaxos-${wc_label}-$$.conf"
    make_config "$proto_config" "$writes"

    log "=== Protocol: epaxos/Orca (weakRatio=0, writes=$writes) ==="
...
log ""
log "Phase 107 complete! Results in $BASE_DIR/"
