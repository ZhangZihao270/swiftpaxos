...
# Phase 89: 5-Replica 5-Machine, 3-Client Validation
#
# 5 replicas on 5 separate machines (1 per machine):
#   .101 (64-core): replica0, client0
#   .103 (64-core): replica1, client1
#   .104 (64-core): replica2, client2
#   .125 (8-core):  replica3 (no client)
#   .126 (8-core):  replica4 (no client)
#
# Tests per-client bottleneck theory: with 3 clients, throughput should
# cap at ~42K (3 × ~14K/client) if per-client is the bottleneck.
#
# Usage: bash scripts/eval-phase89.sh [output-dir]

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase89-$DATE}"
EXP_DIR="$BASE_DIR/exp3.1"
THREAD_COUNTS=(8 32 64 128)
MAX_RETRIES=2

# Use the 5-replica 3-client config
CONFIG="/tmp/eval-phase89-$$.conf"
...
log "Phase 89: 5-Replica 5-Machine, 3-Client — CURP-HT"
...
log ""
log "Phase 89 complete! Results: $BASE_DIR/summary-exp3.1.csv"
