...
#
# Same setup as Phase 94, but with writes=50 (was 5).
# Protocols: curp-baseline, curpht, curpho
# Thread counts: 1, 2, 4, 8, 16, 32, 64, 96
# 5 replicas on 5 machines, 3 clients on .101/.103/.104
# reqs=3000, networkDelay=25, commandSize=100, writes=50
#
# Usage: bash scripts/eval-phase95.sh [output-dir] [protocol-filter]

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase95-$DATE}"
PROTO_FILTER="${2:-all}"
...
# Key change from Phase 94: writes=50, weakWrites=50 (was 5)
declare -a ALL_PROTOCOLS=(
    "curp-baseline:curp:0:50:50"
    "curpht:curpht:50:50:50"
    "curpho:curpho:50:50:50"
)
...
log "Phase 95: Exp 3.1 with 50% Write Ratio (5r/5m/3c)"
...
for proto_spec in "${PROTOCOLS[@]}"; do
    ...
    proto_config="/tmp/eval-phase95-${name}-$$.conf"
...
log ""
log "Phase 95 complete! Results: $BASE_DIR/summary-exp3.1.csv"
