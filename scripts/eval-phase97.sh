...
# Phase 97: Exp 1.1 — Raft-HT vs Vanilla Raft (5r/5m/3c)
#
# Throughput vs latency curves for Raft-HT and Vanilla Raft.
...
#
# Usage: bash scripts/eval-phase97.sh [output-dir] [write-filter] [protocol-filter]
...
DATE=$(date +%Y%m%d)
BASE_DIR="${1:-results/eval-5r5m3c-phase97-$DATE}"
WRITE_FILTER="${2:-all}"
PROTO_FILTER="${3:-all}"
...
log "Phase 97: Exp 1.1 — Raft-HT vs Vanilla Raft (5r/5m/3c)"
...
for wc_spec in "${WRITE_CONFIGS[@]}"; do
    ...
    for proto_spec in "${PROTOCOLS[@]}"; do
        ...
        proto_config="/tmp/eval-phase97-${name}-${wc_label}-$$.conf"
...
log ""
log "Phase 97 complete! Results in $BASE_DIR/"
