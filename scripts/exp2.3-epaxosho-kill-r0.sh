...
# Exp 2.3 Phase 113g: EPaxos-HO Failure Recovery — Kill Co-Located Replica
#
# Kills replica0 (130.245.173.101) which has client0 co-located.
# Tests client failover when its local replica dies.
#
# Expected: client0 fails over to next closest replica, throughput drops
# by ~33% (client0's ops now go through a remote replica).
#
# Usage: bash scripts/exp2.3-epaxosho-kill-r0.sh [output-dir] [kill-delay-s]

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d_%H%M%S)
BASE_DIR="${1:-results/exp2.3-epaxosho-killr0-$DATE}"
KILL_DELAY="${2:-60}"
STARTUP_DELAY=25
THREADS=16

ALL_HOSTS=(130.245.173.101 130.245.173.103 130.245.173.104 130.245.173.125 130.245.173.126)
KILL_HOST="130.245.173.101"  # replica0 — client0 co-located
...
CONFIG="/tmp/exp2.3-epaxosho-killr0-$$.conf"
...
log "Exp 2.3 Phase 113g: EPaxos-HO Failure Recovery — Kill Co-Located Replica"
log "Config: epaxosho, t=$THREADS, reqs=100000, writes=50%, weakRatio=50%"
log "Kill delay: ${KILL_DELAY}s after clients start"
log "Kill target: $KILL_REPLICA on $KILL_HOST (client0 co-located)"
log "Output: $BASE_DIR"
echo ""
...
log ""
log "Exp 2.3 Phase 113g complete! Results in $BASE_DIR/"
