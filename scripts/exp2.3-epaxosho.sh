...
# Exp 2.3: EPaxos-HO Failure Recovery (5r/5m/3c)
#
# Measures per-second throughput with a replica kill at t=60s.
# EPaxos-HO is leaderless, so killing any replica should NOT cause total outage —
# throughput should degrade gracefully (lose ~20% capacity from 5→4 replicas)
# and recover quickly (<1s, no election needed).
#
# Output: TPUT lines in client logs for time-series analysis.
#
# Usage: bash scripts/exp2.3-epaxosho.sh [output-dir] [kill-delay-s]
#   kill-delay-s: seconds after clients start before killing replica (default: 60)

WORK_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$WORK_DIR"

DATE=$(date +%Y%m%d_%H%M%S)
BASE_DIR="${1:-results/exp2.3-epaxosho-$DATE}"
KILL_DELAY="${2:-60}"
STARTUP_DELAY=25
THREADS=16

ALL_HOSTS=(130.245.173.101 130.245.173.103 130.245.173.104 130.245.173.125 130.245.173.126)
KILL_HOST="130.245.173.101"  # replica0 — client0 is co-located here
...
CONFIG="/tmp/exp2.3-epaxosho-$$.conf"
...
# Total delay = startup_delay + kill_delay (from script start) + setup overhead
TOTAL_KILL_DELAY=$((STARTUP_DELAY + KILL_DELAY + 20))  # +20 for sync/setup overhead
log "Scheduling replica0 kill in ${TOTAL_KILL_DELAY}s (startup=${STARTUP_DELAY} + overhead=20 + kill_delay=${KILL_DELAY})"

(
    sleep "$TOTAL_KILL_DELAY"
    echo ""
    echo "[$(date '+%H:%M:%S')] *** KILLING replica0 on $KILL_HOST ***"
    # Kill only the server process, NOT the co-located client
    ssh $SSH_OPTS "$KILL_HOST" "pkill -9 -f '$BINARY -run server'" 2>/dev/null || true
    echo "[$(date '+%H:%M:%S')] *** replica0 killed ***"
) &
KILL_PID=$!
...
log ""
log "Exp 2.3 EPaxos-HO complete! Results in $BASE_DIR/"
