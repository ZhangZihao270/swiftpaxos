#!/bin/bash
# Phase 32.4: Test Network Batching with Different Delays
# Test CURP-HT with batch delays: 0, 50, 75, 100, 125, 150, 200μs
...
ITERATIONS="${1:-3}"  # 3 iterations per delay value

mkdir -p "$OUTPUT_DIR"

TIMESTAMP=$(date +%Y%m%d-%H%M%S)
RESULT_FILE="$OUTPUT_DIR/phase-32.4-batch-delay-sweep-$TIMESTAMP.txt"

# Batch delays to test (in microseconds)
DELAYS=(0 50 75 100 125 150 200)

echo "========================================" | tee "$RESULT_FILE"
echo "Phase 32.4: CURP-HT Batch Delay Sweep" | tee -a "$RESULT_FILE"
...
echo "  - Delays to test: ${DELAYS[*]} μs" | tee -a "$RESULT_FILE"
...
for delay in "${DELAYS[@]}"; do
    ...
    for i in $(seq 1 $ITERATIONS); do
        ...
        echo "    Strong median: ${STRONG_MEDIAN}ms, Weak median: ${WEAK_MEDIAN}ms" | tee -a "$RESULT_FILE"
...
# Final summary
echo "========================================" | tee -a "$RESULT_FILE"
echo "Phase 32.4 Summary: Batch Delay Comparison" | tee -a "$RESULT_FILE"
echo "========================================" | tee -a "$RESULT_FILE"
echo "" | tee -a "$RESULT_FILE"
...
echo "" | tee -a "$RESULT_FILE"
echo "========================================" | tee -a "$RESULT_FILE"
echo "Batch delay sweep complete!" | tee -a "$RESULT_FILE"
