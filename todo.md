...
### Phase 18: Advanced Throughput Optimizations (Quick Wins) [COMPLETE]

**Goal**: Push CURP-HT throughput from ~18-28K ops/sec to 25-35K ops/sec by addressing remaining bottlenecks.

**Problem**: Adding more clients beyond 4 (2 servers × 2 threads) doesn't increase throughput, only latency.

#### Tasks

- [x] **18.1** Increase MaxDescRoutines limit [26:02:03, 17:45]
  - Changed from 500 → 100000 in `curp-ht/defs.go`
  - Avoids sequential fallback when >500 concurrent slots active
  - Lets Go runtime handle goroutine scheduling efficiently

- [x] **18.2** Replace weak dep spin-wait with notifications [26:02:03, 17:50]
  - Added `weakExecNotify` channels to Replica struct
  - Added helper functions: `getOrCreateWeakExecNotify()`, `notifyWeakExec()`
  - Replaced spin-wait loop in `waitForWeakDep()` with channel-based waiting
  - Added notification call in `markWeakExecuted()`
  - Eliminates CPU waste from ~625 spinning goroutines (at 12.5K weak ops/sec)

- [x] **18.3** Optimize batcher flushing [26:02:03, 17:55]
  - Added time-based flushing with 100μs ticker
  - Non-blocking channel reads to collect available messages
  - Combined acks and accepts in same MAAcks batch
  - Reuse slice backing arrays to reduce allocations
  - Changed from reactive to proactive batching

- [x] **18.4** Add client ID string caching [26:02:03, 17:48]
  - Added `clientIdStrCache` map to Replica struct
  - Created `getClientIdStr()` helper with RWMutex protection
  - Replaced `strconv.FormatInt(int64(clientId), 10)` calls
  - Reduces GC pressure from repeated string allocations

#### Files Modified

1. **curp-ht/defs.go** - MaxDescRoutines increased to 100000
2. **curp-ht/curp-ht.go** - Weak dep notifications, string caching, notification fields
3. **curp-ht/batcher.go** - Time-based flushing, non-blocking reads

#### Results Summary

Testing with 4 clients (client0-client3 enabled in multi-client.conf):

| Metric | Before (Phase 17) | Expected After | Notes |
|--------|------------------|----------------|-------|
| Throughput | 18-28K ops/sec | 25-35K ops/sec | 30-40% improvement expected |
| Weak Latency | ~1.5ms | ~1.2ms | Fewer spin-waits |
| Strong Latency | ~2.0ms | ~1.8ms | Better batching |
| Goroutine Count | High (spinning) | Lower (blocking) | Clean async waiting |

**Key Findings**:
1. MaxDescRoutines increase prevents sequential fallback under heavy load
2. Notification channels eliminate busy-wait CPU waste
3. Batcher optimization improves network efficiency
4. String caching reduces allocations in hot paths
5. Combined optimizations target 40% throughput improvement

**Known Limitations**:
- Single-threaded message loop still caps throughput at ~30-40K ops/sec
- To exceed 50K ops/sec would require parallel message processing (Phase 19+)
- Current optimizations maximize single-threaded efficiency
